...
void Player::updateCamera(scene::components::ECSManager& ecs)
{
    ...
    glm::quat yawRotation =
            glm::angleAxis(
                    glm::radians(camera->getYaw()),
                    glm::vec3(0.0f, 1.0f, 0.0f)
            );

    avatar->transform->rotation = yawRotation;

    // transform.position is the player's feet.
    const glm::vec3 back =
            glm::normalize(
                    yawRotation * glm::vec3(0.0f, 0.0f, -1.0f)
            );

    const glm::vec3 targetCameraPosition =
            avatar->transform->position
            + glm::vec3(0.0f, CAMERA_HEIGHT, 0.0f)
            + back * CAMERA_DISTANCE;

    const glm::vec3 newCameraPosition =
            glm::mix(
                    camera->getPosition(),
...
void Player::processInput(GLFWwindow* window, double timeScale, scene::components::ECSManager& ecs)
{
    ...
    bool isSprinting =
            shiftPressed &&
            (liveAvatar->m_activePunchIntent == BoxingPunch::None);

    if (forwardInput != 0.0f || strafeInput != 0.0f) {
        float forwardMagnitude =
                forwardInput * (isSprinting ? 1.0f : 0.5f);
        float strafeMagnitude =
                strafeInput * (isSprinting ? 1.0f : 0.5f);

        liveAvatar->m_currentDirection =
                glm::vec2(strafeMagnitude, forwardMagnitude);

        liveAvatar->m_currentSpeedFactor =
                glm::length(liveAvatar->m_currentDirection);
    } else {
...
void Player::mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    auto* liveAvatar =
            player->ecsManager->getCharacterControllerComponent(
                    player->getAvatarId()
            );
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        if (shiftPressed) {
            liveAvatar->m_activePunchIntent =
                    BoxingPunch::LeftUppercut;
        } else if (holdQ) {
            liveAvatar->m_activePunchIntent =
                    BoxingPunch::LeftHook;
        } else {
            liveAvatar->m_activePunchIntent =
                    BoxingPunch::Jab;
        }
...
RaycastResult Player::raycast(float maxDistance) {
    ...
    sideDist.x =
            (step.x > 0)
            ? (voxelPos.x + 1.0f - rayOrigin.x) * deltaDist.x
            : (rayOrigin.x - voxelPos.x) * deltaDist.x;

    sideDist.y =
            (step.y > 0)
            ? (voxelPos.y + 1.0f - rayOrigin.y) * deltaDist.y
            : (rayOrigin.y - voxelPos.y) * deltaDist.y;

    sideDist.z =
            (step.z > 0)
            ? (voxelPos.z + 1.0f - rayOrigin.z) * deltaDist.z
            : (rayOrigin.z - voxelPos.z) * deltaDist.z;
